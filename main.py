...
TEL_NUMBER = '19923846354'  # 13385736745
TEL_NAME = '沃的发'  # 名字(可选)
...
def iterate_api(file_path):
    ...
    示例路径: r"D:\1\contactPutianHospital-main\api.txt"
    """  # 已正确闭合三引号
    ...
    if ENABLE_OTP:
        ...
        max_workers = 1  # 最大线程数，可根据需要调整
        print(f"使用多线程模式处理URLs，线程数：{max_workers}")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 使用executor.map处理所有URL
            results = executor.map(
                lambda url: process_tab(page, url, success_counter, total_len), 
                urls
            )
            # 遍历结果以确保所有任务完成
            for _ in results:
    ...
    print(f"处理完成，成功: {success_counter['success']}/{total_len}")

if __name__ == '__main__':
    if TEL_NUMBER and TEL_NUMBER.isdigit():
        start_time = time.time()
        iterate_api('api.txt')  # 确保api.txt文件存在于当前目录
        end_time = time.time()
        print(f"结束！总耗时: {end_time - start_time:.2f} seconds")
    else:
        print("请先输入有效的手机号码")
